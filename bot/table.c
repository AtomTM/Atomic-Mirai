...
void table_unlock_val(uint8_t id)
{
    ...
    if (!val->locked)
    {
        printf("(atomic/xor) Tried to double-unlock value %d\n", id);
        return;
    }
#endif
...
void table_lock_val(uint8_t id)
{
    ...
    if (val->locked)
    {
        printf("(atomic/xor) Tried to double-lock value\n");
        return;
    }
#endif
...
char *table_retrieve_val(int id, int *len)
{
    ...
    if (val->locked)
    {
        printf("(atomic/xor) Tried to access table.%d but it is locked\n", id);
        return NULL;
    }
#endif
